...
class ProcessHandler extends EventEmitter{
    ...
    this.tasks = Object.keys(tasks).reduce((handlers, taskName)=>{
      ...
      switch(type){
        case('function'):
          ...
          break;
        case('object'):
          handlers[taskName] = Object.keys(runner).reduce((lib, key)=>{
            const Handler = runner[key];
  ...
  runProcessStep(stepNumber, state, callback){
    ...
    const step = getObjectValue(stepConfig.task, this.tasks);//this.tasks[stepConfig.task];
    if(!step){
    ...
    try{
      ...
      newState.params = defaults({}, stepConfig.params, state.params);
  ...
  execute(params, callback){
    ...
    const nextStep = ({lastCode = [0], lastState = [], stepNumber = 0, params = {}} = {})=>{
      this.runProcessStep(stepNumber, {lastCode, lastState, params}, (code, value)=>{
        ...
        return setImmediate(()=>{
          const nextParams = defaults({}, value, params);
          return nextStep({
            stepNumber: nextStepNumber,
            lastCode: lastCode.concat(code),
            lastState: lastState.concat(value),
            params: nextParams,
          });
        });
      });
